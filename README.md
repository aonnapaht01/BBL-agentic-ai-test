...
- **Dynamic Multi-Provider LLM Factory:** Seamlessly switches between LLM providers via `.env` configuration (Groq, Google Gemini, Azure OpenAI, standard OpenAI) without modifying code.
...
- Python 3.10+
- At least one API key (Groq, Gemini, Azure OpenAI, or OpenAI).
...
1. **Groq (`GROQ_API_KEY`):** Priority 1. Fast & free local testing (defaults to `llama-3.1-8b-instant`).
2. **Google Gemini (`GOOGLE_API_KEY`):** Priority 2. Free local testing (defaults to `gemini-2.0-flash`).
3. **Azure OpenAI (`AZURE_OPENAI_*`):** Priority 3. BBL production / enterprise deployment.
4. **Standard OpenAI (`OPENAI_API_KEY`):** Priority 4. Default fallback (defaults to `gpt-5-mini` or `gpt-4o-mini`).
